...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
    ...
    // Check for published bootcamp for user
...
exports.uploadBootcampPhoto = asyncHandler(async (req, res, next) => {
    ...
    if (file.size > process.env.MAX_FILE_UPLOAD) {
        return next(new ErrorResponse(`Please upload a valid image file less than ${process.env.MAX_FILE_UPLOAD}`, 400));
    }
    ...
    file.mv(`${process.env.FILE_UPLOAD_PATH}/${file.name}`, async err => {
        if (err) {
            console.error(err);
            return next(new ErrorResponse(`There was a problem uploading the file`, 500));
        }
